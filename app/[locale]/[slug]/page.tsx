  ...
  return (
    <main
      className={`min-h-screen ${theme.textColor} flex flex-col items-center justify-between py-12 px-6 bg-gradient-to-b ${theme.mainBg} bg-cover bg-center ${
        backgroundImageUrl ? "profile-bg-overlay" : ""
      }`}
      style={
        backgroundImageUrl
          ? ({
              "--profile-bg-url": `url(${backgroundImageUrl})`,
    ...
    >
      ...
      <div className="w-full max-w-md flex justify-end mb-8">
        <Link
          ...
          className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-sm font-bold ${theme.textColor} transition-all shadow-md backdrop-blur-md animate-premium-enter ${theme.langBtnBg} ${theme.langBtnBorder}`}
        >
      ...
      <div className="flex-1 w-full max-w-md flex flex-col items-center gap-8">
        ...
        {logoUrl ? (
          <img
            ...
            className={`w-28 h-28 rounded-full object-cover border-4 shadow-xl animate-premium-enter ${theme.logoBorder}`}
          />
        ) : (
          <div
            style={{ animationDelay: "150ms" }}
            className={`w-28 h-28 rounded-full bg-slate-800 border-4 flex items-center justify-center shadow-xl animate-premium-enter ${theme.logoBorder}`}
          >
        ...
        <div
          ...
          className="text-center flex flex-col gap-2 animate-premium-enter"
        >
          <h1
            className={`text-3xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r ${theme.companyNameColor}`}
          >
            {locale === "ar"
              ? profile.company_name_ar || profile.company_name_en || slug
              : profile.company_name_en || profile.company_name_ar || slug}
          ...
          {(() => {
            ...
            if (!address || !profile.location) return null;
            ...
            return (
              ...
              >
                <FaMapMarkerAlt
                  className={`w-3.5 h-3.5 ${theme.primaryColor}`}
                />
                <span>{address}</span>
              </a>
            );
        ...
        <div className="w-full flex flex-col gap-4 mt-4">
          {links && links.length > 0 ? (
            links.map((link, idx) => {
              ...
              let href = link.url;
              if (isMobileCall) {
                href = link.url.startsWith("tel:")
                  ? link.url
                  : `tel:${link.url}`;
              } else if (isWhatsApp) {
                const urlTrimmed = link.url.trim();
                if (
                  urlTrimmed.startsWith("http://") ||
                  urlTrimmed.startsWith("https://") ||
                  urlTrimmed.startsWith("wa.me")
                ) {
                  href = urlTrimmed.startsWith("wa.me")
                    ? `https://${urlTrimmed}`
                    : urlTrimmed;
                } else {
              ...
              return (
                <a
                  ...
                  className={`group w-full py-4 px-6 border rounded-2xl font-bold transition-all hover:scale-[1.01] active:scale-[0.99] shadow-md flex justify-between items-center animate-premium-enter ${theme.linkBg} ${theme.linkBorder} ${theme.linkHoverBorder}`}
                >
                  <div className="flex items-center gap-3">
                    <PlatformIcon
                      name={iconName}
                      className={`w-5 h-5 ${theme.primaryColor} transition-colors`}
                    />
                  ...
                  <FaChevronRight
                    className={`w-3.5 h-3.5 ${theme.primaryColor} group-hover:translate-x-1 rtl:rotate-180 rtl:group-hover:-translate-x-1 transition-all`}
                  />
                </a>
              );
            })
          ...
          {(() => {
            ...
            if (instapayLinks.length === 0 && walletNumbers.length === 0)
              return null;

            return (
              <div
                style={{
                  animationDelay: `${350 + (links ? links.length : 0) * 80 + 100}ms`,
                }}
                className={`w-full p-5 rounded-2xl border backdrop-blur-sm flex flex-col gap-4 mt-6 animate-premium-enter ${theme.payCardBg} ${theme.payCardBorder}`}
              >
                <h3
                  className={`font-extrabold text-sm pb-2 uppercase tracking-wide border-b border-current/10 ${theme.cardTitleColor}`}
                >
                ...
                <div className="flex flex-col gap-3">
                  {instapayLinks.map((link: string, idx: number) => (
                    ...
                    >
                      <div
                        className={`p-2 rounded shrink-0 ${theme.payInstaIconBg} ${theme.payInstaIconText}`}
                      >
                        <FaCreditCard className="w-4 h-4" />
                      </div>
                      <div className="flex flex-col text-left rtl:text-right">
                        <span
                          className={`font-bold text-sm ${theme.textColor}`}
                        >
                          {locale === "ar" ? "إنستا باي" : "Instapay"}{" "}
                          {instapayLinks.length > 1 ? `#${idx + 1}` : ""}
                        </span>
                        <span className={`text-xs ${theme.subTextColor}`}>
                          {locale === "ar"
                            ? "ادفع مباشرة عبر إنستا باي"
                            : "Pay directly via Instapay"}
                        </span>
                      </div>
                    </a>
                  ))}
      ...
      <footer
        style={{
          animationDelay: `${350 + (links ? links.length : 0) * 80 + 180}ms`,
        }}
        className={`w-full text-center text-xs mt-16 animate-premium-enter ${theme.subTextColor}`}
      >
        <p>
          ...
          >
            {locale === "ar" ? "الملك للطباعة" : "King Firm"}
          </a>
        </p>
      </footer>
    </main>
